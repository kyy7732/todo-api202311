...
public class UserController {
    ...
    // @PreAuthorize: 메서드 호출 전에 권한 검사 -> 요청 당시 토큰에 있는 user 정보가 ROLE_COMMON이라는 권한을 가지고 있는지 검사.
    @PreAuthorize("hasRole('ROLE_COMMON')") // /promote의 요청이 들어오면 ROLE에서 COMMON인지 먼저 확인 한다.
    public ResponseEntity<?> promote(
        ...
        try {
            ...
            // 예상하지 못한 예외 처리
            return ResponseEntity.internalServerError() // 500에러
                    .body(e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> loadFile(
        ...
        try {
            ...
            byte[] fileData = FileCopyUtils.copyToByteArray(profileFile);// byte배열로 file정보를 리턴해준다.
    ...
    private MediaType findExtensionAndGetMediaType(String filePath) {
        ...
        // 파일 경로에서 확장자 추출하기
        // c:/todo_upload/slkfdjsljfal_abc.jpg
        String ext
    ...
    public ResponseEntity<?> logout(
        ...
        String result = userService.logout(userInfo); // 카카오 로그인 한 사람인 경우: id, 아니면 null
