...
public class JwtAuthFilter extends OncePerRequestFilter { // 한번 요청시 동작하는 필터
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if(token != null) {
                ...
                authorityList.add(new SimpleGrantedAuthority("ROLE_" + userInfo.getRole().toString())); // ROLE_COMMON, ROLE_PREMIUM, ROLE_ADMIN으로 저장됨
                ...
                AbstractAuthenticationToken auth
                        ...
                        null, // 인증된 사용자의 비밀번호 - 보통 null값 (토큰같은경우 비밀번호는 왠만하면 넣지 않는다)
    ...
    private String parseBearerToken(HttpServletRequest request) {
        ...
        // -- Authorization : Bearer lkdsfjlsa...
        String bearerToken = request.getHeader("Authorization");
        
        // 요청 헤더에서 가져온 토큰은 순수 토큰 값이 아닌
        // 앞에 Bearer가 붙어있으니 이것을 제거하는 작업
        if(StringUtils.hasText(bearerToken) // hasText: null 체크, 길이가 0보다 큰지 체크, 공백이 아닌 문자열이 하나라도 포함되었는지까지 한번에 검증
                && bearerToken.startsWith("Bearer")) { // 토큰 앞에 Bearer가 있으면 인증 또는 인가에 사용되는 토큰이다.
